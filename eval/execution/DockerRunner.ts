...
export class DockerExecutionEnvironment implements ExecutionEnvironment {
  ...
  private getExecutionCommand(language: string): {
    command: string[];
    filename: string;
  } {
    switch (language.toLowerCase()) {
      case "javascript":
      case "js":
        return { command: ["node", "main.js"], filename: "main.js" };
      ...
      case "python":
      case "py":
        return { command: ["python", "main.py"], filename: "main.py" };

      case "python2":
        return { command: ["python2", "main.py"], filename: "main.py" };
      ...
      case "go":
        return { command: ["go", "run", "main.go"], filename: "main.go" };
  ...
  private async parseContainerLogs(
  ...
  static isLanguageSupported(language: string): boolean {
    return DockerExecutionEnvironment.getSupportedLanguages().includes(
  ...
  async validateEnvironment(): Promise<{ valid: boolean; missing: string[] }> {
    try {
      ...
      return { valid: false, missing: ["docker"] };
    }
  }
  ...
  async pullRequiredImages(): Promise<void> {
    ...
    for (const image of uniqueImages) {
      try {
        ...
        throw new Error(
          `Failed to pull Docker image ${image}: ${(error as Error).message}`,
        );
      }
    }
  }
