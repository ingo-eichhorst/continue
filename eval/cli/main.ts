...
program
  ...
  .option(
    "-e, --execution-env <type>",
    "Environment to validate (local|docker|all)",
    "all",
  )
  .action(async (options) => {
...
async function runBenchmark(options: any): Promise<void> {
  ...
  if (!pluginName) {
    ...
    if (availablePlugins.length === 1) {
      ...
      throw new Error(
        "Multiple plugins available. Please specify --plugin option.",
      );
    }
  }
  ...
  if (!plugin) {
    throw new Error(
      `Plugin '${pluginName}' not found. Available: ${engine.listPlugins().join(", ")}`,
    );
  }
  ...
  if (options.continue) {
    ...
    logger.info(
      `Progress: ${session.progress.completedTestCases}/${session.progress.totalTestCases} completed`,
    );
  ...
  if (options.dryRun) {
    ...
    logger.info(
      `  Dataset: ${dataset.name} (${dataset.testCases.length} test cases)`,
    );
    logger.info(`  Execution: ${executionEnv.type}`);
    logger.info(
      `  Total test cases: ${models.length * dataset.testCases.length}`,
    );
    return;
  }
...
async function loadModels(
  ...
  try {
    ...
    logger.error(
      "Failed to load models from Continue configuration",
      error as Error,
    );
    throw error;
  }
...
function createExecutionEnvironment(
  type: string,
  logger: ConsoleLogger,
): LocalExecutionEnvironment | DockerExecutionEnvironment {
  switch (type) {
    case "local":
...
async function validateExecutionEnvironment(
  ...
  if (!validation.valid) {
    logger.error(
      `Execution environment validation failed. Missing: ${validation.missing.join(", ")}`,
    );
...
async function displayResults(
  ...
  console.log(
    `Success Rate: ${chalk.bold(((passed / result.testCases.length) * 100).toFixed(1))}%`,
  );
...
async function listSessions(options: any): Promise<void> {
  ...
  sessions.forEach((session) => {
    ...
    console.log(
      `${chalk.cyan(session.id.substring(0, 8))} ${status} ${session.pluginName} (${session.startTime.toISOString()})`,
    );

    if (options.verbose) {
      console.log(
        `  Progress: ${session.progress.completedTestCases}/${session.progress.totalTestCases}`,
      );
...
async function listModels(options: any): Promise<void> {
  ...
  try {
    ...
    if (availableModels.length === 0) {
      ...
      console.log(
        chalk.gray(
          "Make sure you have a .continue/config.json or .continue/config.ts file.",
        ),
      );
      return;
    }
    ...
    if (options.verbose) {
      ...
      console.log(
        chalk.gray("  npm run benchmark -- benchmark --models <model-name>"),
      );
    }
  } catch (error) {
    console.error(
    ...
    console.log(
      chalk.gray(
        "Falling back to default models: gpt-4, gpt-3.5-turbo, claude-3-sonnet",
      ),
    );
  }
}
...
async function listDatasets(options: any): Promise<void> {
  ...
  try {
    ...
    if (availableDatasets.length === 0) {
      ...
      console.log(
        chalk.gray(
          "Create datasets in ./datasets/ folder with dataset.json files.",
        ),
      );
      return;
    }
    ...
    for (const datasetName of availableDatasets) {
      try {
        ...
        if (options.verbose) {
          ...
          console.log(
            `  Difficulty: ${dataset.metadata?.difficulty || "unknown"}`,
          );
        ...
        console.log(
          `${chalk.red(datasetName)}: ${chalk.gray("Error loading dataset")}`,
        );
        if (options.verbose) {
    ...
    if (options.verbose) {
      ...
      console.log(
        chalk.gray("  npm run benchmark -- benchmark --dataset <dataset-name>"),
      );
    }
  } catch (error) {
    console.error(
      chalk.red("Failed to list datasets:"),
      (error as Error).message,
    );
  }
}
